...
void 
Hierarchy::Instanceinfo::link_child(const sid_t child_sid, 
				    const std::string child_nick) throw(std::logic_error) {
...
std::string
Hierarchy::Instanceinfo::get_child_nick(const sid_t child_sid) const throw(std::logic_error){
...
sid_t
Hierarchy::Instanceinfo::get_child_sid(const std::string child_nick) const throw(std::logic_error){
...
Hierarchy::Instanceinfo::get_children_nicks() const throw(){
...
bool Hierarchy::Instanceinfo::is_child(const std::string child_nick) const throw(){
  nick2sid_map_t::const_iterator n2s_it = 
    nick2sid.find(child_nick);
...
std::vector<sid_t>
Hierarchy::Instanceinfo::get_brothers() const throw(){
...
const Pointer<Hierarchy::Instanceinfo>
Hierarchy::get_instanceinfo(const sid_t sid) throw(std::logic_error){
  MutexLock l(pimpl->monitor_lock);
...
Pointer<Isc> Hierarchy::get_isc(const sid_t sid) throw(){
  MutexLock l(pimpl->monitor_lock);
...
void Hierarchy::start_i() throw(std::runtime_error){
  Pointer<Hierarchy::Instanceinfo> root_instance(new Hierarchy::Instanceinfo(ROOT,
									     Pointer<Instanceinfo>::Pointer(0),
									     Pointer<Schemafactory>::Pointer(0)));
...
Pointer<Hierarchy::Instanceinfo> 
Hierarchy::get_instanceinfo_i(const sid_t sid) throw(std::logic_error){
  std::map<sid_t,Pointer<Hierarchy::Instanceinfo> >::iterator i_it;
  ...
  Pointer<Hierarchy::Instanceinfo> instanceinfo(i_it->second);
  ...
  for (ci_it = instanceinfo->factory->get_children_interface_infos().begin();
    ...
    Pointer<Schemafactory> child_factory = 
      get_factory(ci_it->name,
		  instanceinfo->factory->get_child_branch_preferences(ci_it->nick));
    ...
    Pointer<Hierarchy::Instanceinfo> child_instanceinfo(new Hierarchy::Instanceinfo(pimpl->next_sid++,
										    instanceinfo,
										    child_factory));
...
Hierarchy::get_instance_i(const sid_t sid) throw(std::logic_error){
  Pointer<Hierarchy::Instanceinfo> instanceinfo(get_instanceinfo_i(sid));
...
Pointer<Isc> Hierarchy::get_isc_i(const sid_t sid) throw(){
...
Hierarchy::load_factory(const std::string file_path) throw(){
...
Hierarchy::get_factory(const std::string interface_name, 
		       const std::vector<std::string>& branch_preferences) const throw(){
...
Hierarchy::get_factories() const throw(){
  return factories;
}
