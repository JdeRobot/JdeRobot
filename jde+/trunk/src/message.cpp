...
Perception::Perception(const sid_t src,
  ...
  :Message(src,dst),
   perception_name(name),perception_data(perception) {}

Perception::~Perception() {}

std::ostream& Perception::put(std::ostream& os) const {
  os << 'P' << src << ',' << dst << ',' << perception_name
     << '(' << perception_data.get_size() << ')';
...
Modulation::Modulation(const sid_t src,
    ...
    :Message(src,dst),
     modulation_name(name),modulation_data(modulation) {}

Modulation::~Modulation() {}

std::ostream& Modulation::put(std::ostream& os) const {
  os << 'M' << src << ',' << dst << ',' << modulation_name
     << '(' << modulation_data.get_size() << ')';
...
std::ostream& State_Change::put(std::ostream& os) const {
  std::string cstate = state_to_str(change_state);
...
std::ostream& New_State::put(std::ostream& os) const {
  std::string nstate = state_to_str(new_state);
