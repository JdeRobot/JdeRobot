...
class PiBot:
	...
	class Odometry:
		...
		def readOdometry(self, kill_event, selfpadre):
			...
			DistRuedas = 0.123 #metros
			Rrueda = 0.032 #Radio de la rueda
			...
			EncoderL = 6
			EncoderR = 24
			sample_time = 0.019 #Intervalo entre mediciones
	...
	def girarHasta(self, angle):
		'''
		Gira el robot hasta una posicion angular dada
		'''
		finish = False
		(x0, y0, t0) = self.getOdometry()
		while(not finish):
