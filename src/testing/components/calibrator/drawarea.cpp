		...
		if (strcmp(word1,"worldline")==0){
			if (number == 9){
				if (numlines <MAX_LINES){
					...
					printf("VisorNect: error, too much lines in the world file configuration.\n");
				}
			}
			else{
				printf("VisorNect: error, worldfile line not valid: %s\n",buffer_file);
			}
		}
			...
			if (number==11){
				if (post_p < MAX_POST){
					...
					printf("VisorNect: error, too much posts in the world file configuration.\n");
				}
			}
			else{
				printf("VisorNect: error, post line not valid: %s\n",buffer_file);
			}
		}
			...
			if (number==11){
				if (jamb_p<MAX_JAMB){
					...
					printf("VisorNect: error, too much jambs in the world file configuration.\n");
				}
			}
			else{
				printf("VisorNect: error, post line not valid: %s\n",buffer_file);
			...
			if (number==4){
				if (circle_p<MAX_CIRCLE){
					...
					printf("VisorNect: error, too much circles in the world file configuration.\n");
				}
			}
			else{
				printf("VisorNect: error, circle line not valid: %s\n",buffer_file);
			}
		}
			...
			if (number==4){
				if (triangle_p<MAX_TRIANGLE){
					...
					printf("VisorNect: error, too much triangles in the world file configuration.\n");
				}
			}
			else{
				printf("VisorNect: error, triangle line not valid: %s\n",buffer_file);
			}
		}
			...
			if (number==4){
				if (sq_p<MAX_SQUARE){
					...
					printf("VisorNect: error, too much sqs in the world file configuration.\n");
				}
			}
			else{
				printf("VisorNect: error, sq line not valid: %s\n",buffer_file);
			}
		}
	}
	return 1;
...
DrawArea::read_world_file(char* worldfile){
	...
	if ((myfile=fopen(worldfile,"r"))==NULL){
		printf("VisorNect: cannot find config file\n");
		return -1;
	}
...
void DrawArea::draw_camera(double ppx, double ppy, int camera){
  
  /* Estos incrementos se usan para obtener el siguiente
     punto*/
  float inc_x = 1./IMAGE_WIDTH;
  float inc_y = 1./IMAGE_HEIGHT;
  float _z = -1.0;
