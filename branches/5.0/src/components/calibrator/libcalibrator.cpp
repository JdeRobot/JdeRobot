...
Display *mydisplay;
static int myscreen;
FD_calibratorgui *fd_calibratorgui;
/* buffer de imagen */
...
int lower_x,lower_y,greater_x,greater_y;

/* X Image variables */
GC libcalibrator_gc;
Window libcalibrator_win;
XImage *libcalibrator_imagen;
XImage *virtual_libcalibrator_imagen;
XGCValues gc_values;
...
void save_cam(char *fich_sal)
{
  ...
  fprintf(salida,"#libcalibrator, position\n");
...
void load_cam(char *fich_in)
{
  ...
  entrada=fopen(fich_in,"r");
   if(entrada==NULL){
     printf("libcalibrator: camera input calibration file %s does not exits\n",fich_in);
   }else{
...
void libcalibrator_terminate(){
    if (fd_calibratorgui!=NULL)
    {
      fl_hide_form(fd_calibratorgui->calibratorgui);
      fl_free_form(fd_calibratorgui->calibratorgui);
    }
}

void libcalibrator_init(const char *wordfile,const char *camIn,const char *camOut)
{
  int i=0;
  FILE *myconfig;

   /* initializes the 3D world */
   myconfig=fopen(wordfile,"r");
   if(myconfig==NULL){
     printf("libcalibrator: world configuration configuration file %s does not exits\n",wordfile);
   }else{
     printf("calibrator: world configuration file %s\n",wordfile);

     strncpy(cameraINfile,camIn,strlen(camIn));
     strncpy(cameraOUTfile,camOut,strlen(camOut));

     printf("calibrator: int configuration file %s\n",cameraINfile);
     printf("calibrator: out configuration file %s\n",cameraOUTfile);
...
void set_sliders_position()
{
  ...
  fl_set_slider_value(fd_calibratorgui->x_cam_slider,(double)mycameras[mycam].position.X);
  fl_set_slider_value(fd_calibratorgui->y_cam_slider,(double)mycameras[mycam].position.Y);
  fl_set_slider_value(fd_calibratorgui->z_cam_slider,(double)mycameras[mycam].position.Z);

  /* orientation: foa + roll */
  fl_set_slider_value(fd_calibratorgui->X_cam_slider,(double)mycameras[mycam].foa.X);
  fl_set_slider_value(fd_calibratorgui->Y_cam_slider,(double)mycameras[mycam].foa.Y);
  fl_set_slider_value(fd_calibratorgui->Z_cam_slider,(double)mycameras[mycam].foa.Z);
  fl_set_slider_value(fd_calibratorgui->roll_slider,(double)((mycameras[mycam].roll*360)/(2*PI)));

  /* intrinsics */
  fl_set_slider_value(fd_calibratorgui->focus_slider,(double)mycameras[mycam].fdisty);
  fl_set_slider_value(fd_calibratorgui->u0,(double)mycameras[mycam].u0);
  fl_set_slider_value(fd_calibratorgui->v0,(double)mycameras[mycam].v0);
}


void libcalibrator_guibuttons(){
  ...
  if (obj == fd_calibratorgui->exit_button){ /* EXIT BUTTON */
    libcalibrator_terminate();
    exit(0);

  }else if (obj == fd_calibratorgui->flip_button){ /* FLIP BUTTON */
    if (fl_get_button(fd_calibratorgui->flip_button)==RELEASED){
      flip_images = 0;
    }else{
      flip_images = 1;
    }
  }
  else if (obj == fd_calibratorgui->fixedFOA){ /* FixedFOA BUTTON */
    if (fl_get_button(fd_calibratorgui->fixedFOA)==RELEASED){
      fixedFOA = 0;
    }else{
      fixedFOA = 1;
    }
    
  }else if (obj == fd_calibratorgui->save){ /* EXPORT BUTTON */
      save_cam(cameraOUTfile);
  }else  if (obj == fd_calibratorgui->reload){ /* RELOAD BUTTON */
      load_cam(cameraINfile);
  }else if (obj == fd_calibratorgui->undo){ /* UNDO BUTTON */
      load_cam(cameraOUTfile);

  }else if((obj == fd_calibratorgui->x_cam_slider)||(obj == fd_calibratorgui->y_cam_slider)||(obj == fd_calibratorgui->z_cam_slider)){
    /* CAMERA POSITION SLIDERS */
    deltaX=(float) fl_get_slider_value(fd_calibratorgui->x_cam_slider)-mycameras[mycam].position.X;
    deltaY=(float) fl_get_slider_value(fd_calibratorgui->y_cam_slider)-mycameras[mycam].position.Y;
    deltaZ=(float) fl_get_slider_value(fd_calibratorgui->z_cam_slider)-mycameras[mycam].position.Z;
    ...
    mycameras[mycam].foa.X=(float)fl_get_slider_value(fd_calibratorgui->X_cam_slider);
    mycameras[mycam].foa.Y=(float)fl_get_slider_value(fd_calibratorgui->Y_cam_slider);
    mycameras[mycam].foa.Z=(float)fl_get_slider_value(fd_calibratorgui->Z_cam_slider);
    mycameras[mycam].foa.H=1.;
    update_camera_matrix(&mycameras[mycam]);
    /*mydebug();*/
    
  }else if(obj == fd_calibratorgui->roll_slider){ /* ROLL SLIDER */
    mycameras[mycam].roll=(2.*PI*(float) fl_get_slider_value(fd_calibratorgui->roll_slider))/360;
    update_camera_matrix(&mycameras[mycam]); 
    /*mydebug();*/

  }else if(obj == fd_calibratorgui->cam_joystick){

    a=fl_get_positioner_xvalue(fd_calibratorgui->cam_joystick);
    if ((a<0.2)&&(a>-0.2)) a=0.;
    a=a*a*a*3;
    inc_longi=2*PI*a/360.;
    b=fl_get_positioner_yvalue(fd_calibratorgui->cam_joystick);
    if ((b<0.2)&&(b>-0.2)) b=0.;
    b=b*b*b*3;
    inc_lati=2*PI*b/360.;
    ...
    fl_set_slider_value(fd_calibratorgui->Y_cam_slider,(double)mycameras[mycam].foa.Y);
    fl_set_slider_value(fd_calibratorgui->Z_cam_slider,(double)mycameras[mycam].foa.Z);
    fl_set_positioner_xvalue(fd_calibratorgui->cam_joystick,(double) 0.);
    fl_set_positioner_yvalue(fd_calibratorgui->cam_joystick,(double) 0.);
  
  }else if(obj == fd_calibratorgui->focus_slider){ /* FOCUS DISTANCE SLIDER */

    mycameras[mycam].fdisty=(float) fl_get_slider_value(fd_calibratorgui->focus_slider);
    mycameras[mycam].fdistx=(float) fl_get_slider_value(fd_calibratorgui->focus_slider);
    update_camera_matrix(&mycameras[mycam]);
    /*mydebug();*/

  }else if(obj == fd_calibratorgui->u0){ 

    mycameras[mycam].u0=(float) fl_get_slider_value(fd_calibratorgui->u0);
    update_camera_matrix(&mycameras[mycam]);
    /*mydebug();*/

  }else if(obj == fd_calibratorgui->v0){

    mycameras[mycam].v0=(float) fl_get_slider_value(fd_calibratorgui->v0);
    update_camera_matrix(&mycameras[mycam]);
    /*mydebug();*/
  }
}
...
void libcalibrator_guidisplay(unsigned char *imagesrc){
  int i,fila,columna;
  HPoint2D a,b;
  ...
  fl_redraw_object(fd_calibratorgui->cam_joystick);

  if(mycam == 0){
      bcopy(imagesrc,mybuffer,SIFNTSC_ROWS*SIFNTSC_COLUMNS*3);
  }
 ...
 /*cameras axis*/
  ...
  for(i=0; i<SIFNTSC_ROWS*SIFNTSC_COLUMNS; i++){ 	
    ...
    image_buf[i*4+3]=255; /* dummy byte */ 
    ...
    virtual_image_buf[i*4+3]=255; /* dummy byte */ 
  };

  /* pintamos la imagen modificada */
  XPutImage(mydisplay,libcalibrator_win,libcalibrator_gc,libcalibrator_imagen,0,0,fd_calibratorgui->color_freeobject->x, fd_calibratorgui->color_freeobject->y,  SIFNTSC_COLUMNS, SIFNTSC_ROWS);

  XPutImage(mydisplay,libcalibrator_win,libcalibrator_gc,virtual_libcalibrator_imagen,0,0,fd_calibratorgui->virtual_freeobject->x, fd_calibratorgui->virtual_freeobject->y,  SIFNTSC_COLUMNS, SIFNTSC_ROWS);
}
...
void libcalibrator_initgui(void)
{
  int vmode,i;
  static XGCValues gc_values;
  int myargc=1; char **myargv; char *aa; char a[]="myjde";
	
     /* preparing display */
     aa=a; myargv=&aa;
     mydisplay= fl_initialize(&myargc,myargv,"JDE",0,0); /* 0,0 can be change to initial position of the main form*/
     fd_calibratorgui = create_form_calibratorgui();
     fl_set_form_position(fd_calibratorgui->calibratorgui,400,50);
     fl_show_form(fd_calibratorgui->calibratorgui,FL_PLACE_POSITION,
		  FL_FULLBORDER,"calibrator");
     fl_set_form_atclose(fd_calibratorgui->calibratorgui,libcalibrator_hidegui,0);
     
     libcalibrator_win = FL_ObjWin(fd_calibratorgui->color_freeobject);

     /* Inicializa las ventanas, la paleta de colores y memoria compartida para visualizacion*/
     gc_values.graphics_exposures = False;
     libcalibrator_gc = XCreateGC(mydisplay,libcalibrator_win, GCGraphicsExposures, &gc_values);
     ...
     if ((vmode==TrueColor)&&(fl_state[vmode].depth==16))
       {
	 printf("16 bits mode\n");
	 libcalibrator_imagen = XCreateImage(mydisplay,DefaultVisual(mydisplay,myscreen),16,ZPixmap,0,image_buf,SIFNTSC_COLUMNS,SIFNTSC_ROWS,8,0);
	 virtual_libcalibrator_imagen = XCreateImage(mydisplay,DefaultVisual(mydisplay,myscreen),16,ZPixmap,0,virtual_image_buf,SIFNTSC_COLUMNS,SIFNTSC_ROWS,8,0);
       }
     else if ((vmode==TrueColor)&&(fl_state[vmode].depth==24))
       {
	 printf("24 bits mode\n");
	 libcalibrator_imagen = XCreateImage(mydisplay,DefaultVisual(mydisplay,myscreen),24,ZPixmap,0,image_buf,SIFNTSC_COLUMNS,SIFNTSC_ROWS,8,0);
	 virtual_libcalibrator_imagen = XCreateImage(mydisplay,DefaultVisual(mydisplay,myscreen),24,ZPixmap,0,virtual_image_buf,SIFNTSC_COLUMNS,SIFNTSC_ROWS,8,0);
       }
     else if ((vmode==TrueColor)&&(fl_state[vmode].depth==32))
       {
	 printf("32 bits mode\n");
	 libcalibrator_imagen = XCreateImage(mydisplay,DefaultVisual(mydisplay,myscreen),32,ZPixmap,0,image_buf,SIFNTSC_COLUMNS,SIFNTSC_ROWS,8,0);
	 virtual_libcalibrator_imagen = XCreateImage(mydisplay,DefaultVisual(mydisplay,myscreen),32,ZPixmap,0,virtual_image_buf,SIFNTSC_COLUMNS,SIFNTSC_ROWS,8,0);  
       }
     else if ((vmode==PseudoColor)&&(fl_state[vmode].depth==8))
       {
	 printf("8 bits mode\n");
	 libcalibrator_imagen = XCreateImage(mydisplay,DefaultVisual(mydisplay,myscreen),8,ZPixmap,0,image_buf,SIFNTSC_COLUMNS,SIFNTSC_ROWS,8,0);
	 virtual_libcalibrator_imagen = XCreateImage(mydisplay,DefaultVisual(mydisplay,myscreen),8,ZPixmap,0,virtual_image_buf,SIFNTSC_COLUMNS,SIFNTSC_ROWS,8,0);
       }
     else
       {
    ...
    fl_show_form(fd_calibratorgui->calibratorgui,FL_PLACE_POSITION,
		 FL_FULLBORDER,"calibrator");
    libcalibrator_win = FL_ObjWin(fd_calibratorgui->color_freeobject);
  /* the window (libcalibrator_win) changes every time the form is hided and showed again. They need to be updated before displaying anything again */

    /* Translation */
  fl_set_slider_bounds(fd_calibratorgui->x_cam_slider,max_room_x,-max_room_x);
  fl_set_slider_value(fd_calibratorgui->x_cam_slider,(double)0.);
  fl_set_slider_bounds(fd_calibratorgui->y_cam_slider,max_room_y,-max_room_y);
  fl_set_slider_value(fd_calibratorgui->y_cam_slider,(double)0.);
  fl_set_slider_bounds(fd_calibratorgui->z_cam_slider,max_room_z,-max_room_z);
  fl_set_slider_value(fd_calibratorgui->z_cam_slider,(double)0.);
  fl_set_slider_bounds(fd_calibratorgui->X_cam_slider,max_room_x,-max_room_x);
  fl_set_slider_value(fd_calibratorgui->X_cam_slider,(double)0.);
  fl_set_slider_bounds(fd_calibratorgui->Y_cam_slider,max_room_y,-max_room_y);
  fl_set_slider_value(fd_calibratorgui->Y_cam_slider,(double)0.);
  ...
  fl_set_positioner_xbounds(fd_calibratorgui->cam_joystick,-1.,1.);
  fl_set_positioner_xvalue(fd_calibratorgui->cam_joystick,(double) 0.);
  fl_set_positioner_ybounds(fd_calibratorgui->cam_joystick,-1,1); 
  fl_set_positioner_yvalue(fd_calibratorgui->cam_joystick,(double) 0.);

  /* Intrinsic parameters: focal distance,etc */
  fl_set_slider_bounds(fd_calibratorgui->focus_slider,(double)FDIST_MAX,(double)FDIST_MIN);
  fl_set_slider_value(fd_calibratorgui->focus_slider,(double)ISIGHT_PINHOLE_FDIST);
  fl_set_slider_bounds(fd_calibratorgui->u0,(double)SIFNTSC_ROWS,(double)0);
  fl_set_slider_value(fd_calibratorgui->u0,(double)0);
  fl_set_slider_bounds(fd_calibratorgui->v0,(double)SIFNTSC_COLUMNS,(double)0);
  fl_set_slider_value(fd_calibratorgui->v0,(double)0);

  /* Rotation angles */
  fl_set_slider_bounds(fd_calibratorgui->roll_slider,180.,-180.);
  fl_set_slider_value(fd_calibratorgui->roll_slider,(double)0.);
...
int virtual_handle(FL_OBJECT *obj, int event, FL_Coord mx, FL_Coord my,int key, void *xev){
  ...
  if(event==FL_PUSH){ /* EVENT IS TO CLICK WITH A MOUSE BUTTON */
    ...
    if((key!=5)&&(key!=4)){ /* ONLY IF CLICKED WITH LEFT, CENTER OR RIGHT MOUSE BUTTON */
      int x,y;
      x=mx-fd_calibratorgui->virtual_freeobject->x+1;
      y=my-fd_calibratorgui->virtual_freeobject->y+1;
 ...
 HPoint3D newpos,ZaxisRel;
  ...
  if(event==FL_PUSH){
    ...
    if((key!=5)&&(key!=4)){
            ...
            unsigned int x,y;
      /*if(flip_images==0){
	x=mx-fd_calibratorgui->color_freeobject->x+1;
	y=my-fd_calibratorgui->color_freeobject->y+1;
      }else{
	x=SIFNTSC_COLUMNS-1-(mx-fd_calibratorgui->color_freeobject->x+1);
	y=SIFNTSC_ROWS-1-(my-fd_calibratorgui->color_freeobject->y+1);
      }
      */
      /*printf("Click on (%d,%d) of (%d,%d)\n",x,y,SIFNTSC_COLUMNS,SIFNTSC_ROWS);*/
      mouse_pressed=1;
	...
	{
	  /*  int y,z;
	  py=(float) fl_get_slider_value(fd_calibratorgui->y_cam_slider);
	  pz=(float) fl_get_slider_value(fd_calibratorgui->z_cam_slider);
	  py=py+(int)diff_x*(-100);
	  pz=pz+(int)diff_y*100;
	  if(py>=MAX_Z) py=MAX_Z;
	  else if(py<=-MAX_Z) py=-MAX_Z;
	  ...
	  px=(float) fl_get_slider_value(fd_calibratorgui->x_cam_slider);
	  r=(float)sqrt((double)(px*px+py*py+pz*pz));
	  lati=(float)asin(pz/r)*360./(2.*PI);
	  longi=(float)atan2((float)py,(float)px)*360./(2.*PI);
	  fl_set_slider_value(fd_calibratorgui->y_cam_slider,(double)py);
	  fl_set_slider_value(fd_calibratorgui->z_cam_slider,(double)pz);
	  fl_set_positioner_xvalue(fd_calibratorgui->cam_joystick,(double) longi);
	  fl_set_positioner_yvalue(fd_calibratorgui->cam_joystick,(double) lati);
	  fl_set_slider_value(fd_calibratorgui->radious_slider,(double)r);


	  y=(float) fl_get_slider_value(fd_calibratorgui->Y_cam_slider);
	  z=(float) fl_get_slider_value(fd_calibratorgui->Z_cam_slider);
	  y=y+(int)diff_x*(-100);
	  z=z+(int)diff_y*100;
	  if(y>=MAX_Z) y=MAX_Z;
	  else if(y<=-MAX_Z) y=-MAX_Z;
	  ...
	  fl_set_slider_value(fd_calibratorgui->Y_cam_slider,(double)y);
	  fl_set_slider_value(fd_calibratorgui->Z_cam_slider,(double)z);
	  */  
