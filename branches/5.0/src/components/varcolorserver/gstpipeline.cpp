...
namespace varcolorserver {
  ...
  GSTPipeline::GSTPipeline(const Config &cfg) throw(GSTPipelineConfigException)
    :config_(cfg)
  {
    ...
    pixelFormat = searchFormat(config_.format.c_str());
    if (pixelFormat == 0)
      throw GSTPipelineConfigException();
    ...
    if (pixelFormat->format == FORMAT_RGB_888 ){
      caps = gst_caps_new_simple ("video/x-raw-rgb",
				  ...
				  NULL);
    }else if (pixelFormat->format == FORMAT_YUY2){/*or FORMAT_YUYV*/
      caps = gst_caps_new_simple ("video/x-raw-yuv",
				  "format", GST_TYPE_FOURCC, GST_STR_FOURCC(config_.format.c_str()),
				  "width", G_TYPE_INT, config_.width,
				  "height", G_TYPE_INT, config_.height,
				  "framerate", GST_TYPE_FRACTION, 
				  config_.framerateN, config_.framerateD,
      ...
      throw GSTPipelineConfigException();/*not implemented yet*/
    }
